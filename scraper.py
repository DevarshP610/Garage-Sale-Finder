...
import os
os.environ["GEMINI_API_KEY"] = "AIzaSyBpq0QesFQD1f7VrsuyWtGqJ7LoCjfu6-c"
os.environ["DATABASE_URL"]   = "postgresql://postgres:BYYYpJdkiWKLyljzlSzYrJGGnHVupSOn@postgres.railway.internal:5432/railway"
...
def scrape_craigslist(city="winnipeg"):
    url = f"https://winnipeg.craigslist.org/search/gss"
    headers = {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36"
    }
    
    print(f"Scraping Craigslist...")
    response = requests.get(url, headers=headers)
    soup = BeautifulSoup(response.text, "html.parser")
...
- date: in YYYY-MM-DD format (if no date found use next Saturday's date which is 2026-04-25)
